layui.config({
	...
	$.get("/SysModelDocc/list", function(data){

        var newData = data;
        newsList(newData, $('#SysModelDocc-list'),renderData);
    ...
    function renderData(currData  ){
        ...
        if(currData.length != 0){
            for(var i=0;i<currData.length;i++){
                dataHtml += '<tr>'
    +'<td><input name="mid"   type="checkbox"><div class="layui-unselect layui-form-checkbox  "><span>勾选</span><i class="layui-icon"></i></div></td>'

     +'<td   >  '+currData[i].mid+ '</td>'

     +'<td   >  '+currData[i].modelNam+ '</td>'

     +'<td   >  '+currData[i].modelDes+ '</td>'

     +'<td   >  '+currData[i].modelTy+ '</td>'

     +'<td   >  '+currData[i].modelTabId+ '</td>'

     +'<td   >  '+currData[i].modelOptTy+ '</td>'
    ...
    +'<td>'
        +  '<a class="layui-btn layui-btn-mini news_edit" tips="'+currData[i].mid+'"><i class="iconfont icon-edit"></i> 编辑</a>'
        +  '<a class="layui-btn layui-btn-normal layui-btn-mini news_collect"><i class="layui-icon">&#xe600;</i> 收藏</a>'
        +  '<a class="layui-btn layui-btn-danger layui-btn-mini news_del" data-id="'+currData[i].mid+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
        +'</td>'
    +'</tr>';
            }
        }else{
	...
	function newsList(newsData,tableDom ,renderData_){
		...
		function renderDatas() {
            ...
            var child = tableDom.find('tbody input[type="checkbox"][name="mid"]');
            ...
            tableDom.find('.news_edit').each(function( ){
                ...
                $(this).click(function(){
                    var index = layui.layer.open({
                        title : "添加",
                        type : 2,
                        content : "/sys/SysModelDocc-form.html?mid="+id,
            ...
            tableDom.find('.news_collect').each(function( ){
                ...
                $(this).click(function(){
                    var index = layui.layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content : '/sys/SysModelDocc-form.html' ,
                        success : function(layero, index){
                            setTimeout(function(){
                                layui.layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                    tips: 3
